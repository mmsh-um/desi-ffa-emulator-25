...
namin_gal = basedir + "/mock" + mock + "/pota-" + args.prog + "_joined_unreduced_" + target + ".fits"
# hdul = fits.open(namin_gal)
# hdul.info()
# hdul[1].header
# data_gal = hdul[1].data
data_gal = Table.read(namin_gal)
unique_targetids, unique_indices = np.unique(data_gal["TARGETID"], return_index=True)
# Create a new table with only unique rows
data_gal = data_gal[unique_indices]
...
namout_txt = outdir + target + "_" + args.mocktype + "_forFAemu_m" + args.mocknumber + ".dat" 
#RA_cut = [20.0, 40.0]
#dec_cut = [-10.0, 10.0]
RA_cut = [-10.0, 370.0]
dec_cut = [-100.0, 100.0]
